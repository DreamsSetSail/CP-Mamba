"""HMNet: Register + MSFM + Gated Recap + optional Dynamic Prototype (Point 3).

Point 3 options (via ``--opts`` or yaml):
  dynamic_proto_mode: none | A | B | C
  proto_kshot_attn: True/False   # A: K-shot attention aggregation
  proto_refine: True/False       # B: 2-step prototype refinement

Usage::
    python train_coco_proto.py --arch=HMNetAMP_Reg_MSFM_GatedRecap_Proto \\
...
from model.mamba_blocks_reg_gated import RegisteredGatedVSSM
from model.recap_gating import RecapGateNetwork
from model.msfm_support import SupportMSFM
from model.dynamic_prototype import build_dynamic_prototype, baseline_kshot_aggregate
...
class OneModel(nn.Module):
    def __init__(self, args, blocks=None):
        ...
        self.mamba = RegisteredGatedVSSM(
            depths=depths,
            dims=dims,
            mlp_ratio=mlp_ratio,
            num_registers=getattr(args, 'num_registers', 4),
        )
        self.recap_gate_net = RecapGateNetwork(
            hidden_dim=getattr(args, 'recap_gate_hidden', 32),
            num_windows=16,
        )
        self.msfm_support = SupportMSFM(
            in_planes=reduce_dim,
            msfm_type=getattr(args, 'msfm_type', 'FFT'),
        )
        self.dynamic_proto = build_dynamic_prototype(args, reduce_dim, args.shot)
        if self.dynamic_proto is not None and self.dynamic_proto.enabled:
            flags = []
            if self.dynamic_proto.use_kshot_attn:
                flags.append('A:kshot-attn')
            if self.dynamic_proto.use_proto_refine:
                flags.append('B:proto-refine')
            print(f'[DynamicPrototype] enabled: {", ".join(flags)}')
    ...
    def get_optim(self, model, args, LR):
        ...
        params.append({'params': model.recap_gate_net.parameters()})
        if model.msfm_support.msfm is not None:
            params.append({'params': model.msfm_support.parameters()})
        if getattr(model.mamba, 'num_registers', 0) > 0:
            params.append({'params': model.mamba.mix_registers.parameters()})
        if model.dynamic_proto is not None and model.dynamic_proto.enabled:
            params.append({'params': model.dynamic_proto.parameters()})
    ...
    def forward(self, x, s_x, s_y, y_m, cat_idx=None):
        ...
        for i in range(self.shot):
            ...
            shot_fg_list.append(supp_feat_mid[:, i, :, :, :] * mask)
            shot_proto_list.append(Weighted_GAP(supp_feat_mid[:, i, :, :, :], mask))

        shot_fg_stack = torch.stack(shot_fg_list, dim=1)
        shot_proto_stack = torch.stack(shot_proto_list, dim=1)

        if self.dynamic_proto is not None and self.dynamic_proto.enabled:
            supp_feat, supp_pro, _ = self.dynamic_proto.aggregate(
                query_feat, shot_fg_stack, shot_proto_stack,
            )
        else:
            supp_feat, supp_pro, _ = baseline_kshot_aggregate(shot_fg_stack, shot_proto_stack)

        supp_mask = torch.cat(mask_list, dim=1).mean(1, True)
        supp_pro = supp_pro.expand_as(query_feat)

        # MSFM: frequency-decouple aggregated support features (Point 1)
        supp_feat = self.msfm_support(supp_feat)
        ...
        if self.dynamic_proto is not None and self.dynamic_proto.use_proto_refine:
            supp_pro = self.dynamic_proto.refine(supp_pro, query_feat, corr_query_mask)
        ...
        query_feat_m = self.init_merge_query(query_cat)  # bs, 256, 60, 60
        supp_cat = torch.cat([supp_feat, supp_pro, supp_mask], dim=1)  # bs, 512, 60, 60
        supp_feat_m = self.init_merge_supp(supp_cat)  # bs, 256, 60, 60

        recap_gate = self.recap_gate_net(
            query_feat_m, supp_feat_m, supp_pro, corr_query_mask,
        )

        # mamba blocks (adaptive support recap inside GatedVSSM)
        query_feat = self.mamba(query_feat_m, supp_feat_m, recap_gate=recap_gate)
