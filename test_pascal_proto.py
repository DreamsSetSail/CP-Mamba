...
from model import (
    HMNetAMP,
    HMNetAMP_GatedRecap,
    HMNetAMP_MSFM_GatedRecap,
    HMNetAMP_Reg_MSFM_GatedRecap,
    HMNetAMP_Reg_MSFM_GatedRecap_Proto,
)
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='HMNetAMP_Reg_MSFM_GatedRecap_Proto')
...
def prepare_checkpoint_state_dict(checkpoint):
    """Normalize HMNet checkpoint keys for single-GPU load."""
    raw_state = checkpoint.get('state_dict', checkpoint.get('model'))
    state = {k.replace('module.', ''): v for k, v in raw_state.items()}
    # Legacy DDP checkpoints duplicated mix_registers under each SS2D block.
    state = {k: v for k, v in state.items() if 'self_attention._mix_registers' not in k}
    return state
...
def get_model(args):
    ...
    if args.weight:
        ...
        if os.path.isfile(weight_path):
            ...
            new_state_dict = prepare_checkpoint_state_dict(checkpoint)
            model.load_state_dict(new_state_dict)
            if 'optimizer' in checkpoint:
                optimizer.load_state_dict(checkpoint['optimizer'])
            if 'optimizer_swin' in checkpoint:
                optimizer_swin.load_state_dict(checkpoint['optimizer_swin'])
            logger.info("=> loaded checkpoint '{}' (epoch {})".format(weight_path, args.start_epoch))
        else:
